...
public:
    ...
    Value* processBinaryOp(const string& expr) {
        // Find operator (order matters - check longer operators first!)
        vector<pair<string, string>> ops = {
            {" << ", "shl"}, {" >> ", "shr"},  // Shift operators first
            {" == ", "eq"}, {" != ", "ne"}, {" <= ", "le"}, {" >= ", "ge"},
            {" < ", "lt"}, {" > ", "gt"}, 
            {" && ", "logand"}, {" || ", "logor"},  // Logical AND/OR
        ...
        for (const auto& op : ops) {
            ...
            if (pos != string::npos) {
                ...
                else if (op.second == "logand") result = builder.CreateAnd(lVal, rVal);  // Logical AND
                else if (op.second == "logor") result = builder.CreateOr(lVal, rVal);    // Logical OR
                else if (op.second == "bitand") result = builder.CreateAnd(lVal, rVal);  // Bitwise AND
                else if (op.second == "bitor") result = builder.CreateOr(lVal, rVal);    // Bitwise OR
                else if (op.second == "bitxor") result = builder.CreateXor(lVal, rVal);  // Bitwise XOR
                else if (op.second == "shl") result = builder.CreateShl(lVal, rVal);     // Shift left
                else if (op.second == "shr") result = builder.CreateAShr(lVal, rVal);    // Arithmetic shift right
