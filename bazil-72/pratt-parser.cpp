...
public:
    ...
    vector<ASTPtr> parseProgram() {
        ...
        while (currentToken.type != T_EOF) {
            if (isTypeToken(currentToken.type)) {
                // Look ahead to see if next token is identifier (function) or not (variable)
...
int main() {
    ...
    // Token stream for the test program (with semicolons)
    vector<Token> tokens = {
        // Function: int add(int a, int b) { return a + b; }
        {T_INT, "int", 2, 9}, {T_IDENTIFIER, "add", 2, 13}, {T_LPAREN, "(", 2, 16},
        {T_INT, "int", 2, 17}, {T_IDENTIFIER, "a", 2, 21}, {T_COMMA, ",", 2, 22},
        {T_INT, "int", 2, 24}, {T_IDENTIFIER, "b", 2, 28}, {T_RPAREN, ")", 2, 29},
        {T_LBRACE, "{", 2, 31},
        {T_RETURN, "return", 3, 13}, {T_IDENTIFIER, "a", 3, 20}, {T_PLUS, "+", 3, 22},
        {T_IDENTIFIER, "b", 3, 24}, {T_SEMICOLON, ";", 3, 25},  // Semicolon here
        {T_RBRACE, "}", 4, 9},

        // Main function
        {T_MAIN, "main", 6, 9}, {T_LBRACE, "{", 6, 14},
        
        // int x = 42;
        {T_INT, "int", 7, 13}, {T_IDENTIFIER, "x", 7, 17}, {T_ASSIGNOP, "=", 7, 19},
        {T_INTLIT, "42", 7, 21}, {T_SEMICOLON, ";", 7, 23},
        
        // int y = add(10, 20);
        {T_INT, "int", 8, 13}, {T_IDENTIFIER, "y", 8, 17}, {T_ASSIGNOP, "=", 8, 19},
        {T_IDENTIFIER, "add", 8, 21}, {T_LPAREN, "(", 8, 24}, {T_INTLIT, "10", 8, 25},
        {T_COMMA, ",", 8, 27}, {T_INTLIT, "20", 8, 29}, {T_RPAREN, ")", 8, 31},
        {T_SEMICOLON, ";", 8, 32},
        
        // print("Hello, World!");
        {T_PRINT, "print", 9, 13}, {T_LPAREN, "(", 9, 18}, {T_STRINGLIT, "\"Hello, World!\"", 9, 19},
        {T_RPAREN, ")", 9, 36}, {T_SEMICOLON, ";", 9, 37},
        
        // print("Result: ", x + y);
        {T_PRINT, "print", 10, 13}, {T_LPAREN, "(", 10, 18}, {T_STRINGLIT, "\"Result: \"", 10, 19},
        {T_COMMA, ",", 10, 29}, {T_IDENTIFIER, "x", 10, 31}, {T_PLUS, "+", 10, 33},
        {T_IDENTIFIER, "y", 10, 35}, {T_RPAREN, ")", 10, 36}, {T_SEMICOLON, ";", 10, 37},
        
        // if (x > 0) { print("Positive number"); } else { print("Non-positive"); }
        {T_IF, "if", 12, 13}, {T_LPAREN, "(", 12, 16}, {T_IDENTIFIER, "x", 12, 17},
        {T_GT, ">", 12, 19}, {T_INTLIT, "0", 12, 21}, {T_RPAREN, ")", 12, 22},
        {T_LBRACE, "{", 12, 24}, 
        {T_PRINT, "print", 13, 17}, {T_LPAREN, "(", 13, 22},
        {T_STRINGLIT, "\"Positive number\"", 13, 23}, {T_RPAREN, ")", 13, 40},
        {T_SEMICOLON, ";", 13, 41}, 
        {T_RBRACE, "}", 14, 13},
        {T_ELSE, "else", 14, 15}, {T_LBRACE, "{", 14, 20}, 
        {T_PRINT, "print", 15, 17}, {T_LPAREN, "(", 15, 22},
        {T_STRINGLIT, "\"Non-positive\"", 15, 23}, {T_RPAREN, ")", 15, 37},
        {T_SEMICOLON, ";", 15, 38}, 
        {T_RBRACE, "}", 16, 13},
        
        // while (x > 0) { x = x - 1; print("Count: ", x); }
        {T_WHILE, "while", 17, 13}, {T_LPAREN, "(", 17, 19}, {T_IDENTIFIER, "x", 17, 20},
        {T_GT, ">", 17, 22}, {T_INTLIT, "0", 17, 24}, {T_RPAREN, ")", 17, 25},
        {T_LBRACE, "{", 17, 27}, 
        {T_IDENTIFIER, "x", 18, 17}, {T_ASSIGNOP, "=", 18, 19},
        {T_IDENTIFIER, "x", 18, 21}, {T_MINUS, "-", 18, 23}, {T_INTLIT, "1", 18, 25},
        {T_SEMICOLON, ";", 18, 26}, 
        {T_PRINT, "print", 19, 17}, {T_LPAREN, "(", 19, 22},
        {T_STRINGLIT, "\"Count: \"", 19, 23}, {T_COMMA, ",", 19, 32}, {T_IDENTIFIER, "x", 19, 34},
        {T_RPAREN, ")", 19, 35}, {T_SEMICOLON, ";", 19, 36}, 
        {T_RBRACE, "}", 20, 13},
        
        // return 0;
        {T_RETURN, "return", 22, 13}, {T_INTLIT, "0", 22, 20}, {T_SEMICOLON, ";", 22, 21},
